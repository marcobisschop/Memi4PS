...
<%
    ...
    dim sql
    sql = "select (select count(*) from docum where inreto='keuzen'and inreto_id=keuzen.id) as docum, keucod, keuafd.afdnaa, "
    sql = sql & " memi, voltek, keubes, aantal, "
    sql = sql & " isnull((select kin_available from keuzen_inreto where kin_inreto='oa_afronden' and kin_keuzen_fk=keuzen.id and kin_inreto_fk=" & sec_currentuserid() & "), 0) as afrond "
    sql = sql & ", keuzen.id from keuzen left join keuafd on keuafd_id = keuafd.id" 
    sql = sql & " where keusoo_soonaa='b' and defini=1 and control=1 and koppel_id=" & bounum_id
	...
	fields = fields & "$name:=afrond;type:=truefalse;url:=action.asp?action=oa_afronden&keuzen_id=~id~;title:=Afronden;$"
