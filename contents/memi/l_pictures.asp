...
<%
	...
	sql = "select id,arcbes,cast(arcsiz/1024 as varchar) + ' Kb' as arcsiz,volgor,available,'x'as del from dosarc where inreto='memi_fotos' and inreto_id=" & recordid
	...
	f_form_hdr()
	%><table><%
	%><tr><td><% f_filebox rs, "image", "image", 100 %></td><%
	%><tr><td><% f_textbox rs, "volgor", "volgor", 40 %></td><%
	%><tr><td><% f_textbox rs, "arcbes", "arcbes", 350 %></td></tr><%
	%></table><%
