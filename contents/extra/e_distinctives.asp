...
<%
	...
	if viewstate = "save" or viewstate = "savenew" then	
		if ispostback() then
			...
			if not v_valid("^\w",true,fieldvalue(rs,"inreto")) then v_addformerror "inreto"
	...
	function save_rs(rs)
		with rs
			...
			.fields("group") = lcase(fieldvalue(rs,"group"))
			.fields("inreto") = lcase(fieldvalue(rs,"inreto"))
			.fields("inuse") = checkboxvalue(fieldvalue(rs,"inuse"))
		end with
		sec_setsecurityinfo rs 
		putrecordset rs 
	end function
		...
		<td valign="top">
			<table style="" ID="Table2">
				<%
					...
					f_textbox rs,"group","group","300"
					f_textbox rs,"inreto","inreto","300"
					f_checkbox rs,"inuse","inuse"
				%>
			</table>
		</td>
	</tr>
