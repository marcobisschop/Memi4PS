...
<%
	...
	if viewstate = "save" or viewstate = "savenew" then	
		if ispostback() then
			if not v_valid("^\w",true,fieldvalue(rs,"afdnaa")) then v_addformerror "afdnaa"
	...
	function save_rs(rs)
		with rs
			.fields("afdnaa") = fieldvalue(rs,"afdnaa")
		end with
		sec_setsecurityinfo rs 
		putrecordset rs 
	end function
	
	f_header rs.fields("afdnaa")
	f_form_hdr()
%>
	...
	<tr>
		<td valign="top">
			<table style="" ID="Table2">
				<%
					f_textbox rs,"afdnaa","afdnaa","300"
				%>
			</table>
		</td>
	</tr>
