...
<%
	...
	if viewstate = "save" or viewstate = "savenew" then	
		if ispostback() then
			if not v_valid("^\w",true,fieldvalue(rs,"catnaa")) then v_addformerror "catnaa"
	...
	function save_rs(rs)
		with rs
			.fields("catnaa") = fieldvalue(rs,"catnaa")
		end with
		sec_setsecurityinfo rs 
		putrecordset rs 
	end function
	
	f_header rs.fields("catnaa")
	f_form_hdr()
%>
	...
	<tr>
		<td valign="top">
			<table style="" ID="Table2">
				<%
					f_textbox rs,"catnaa","catnaa","300"
				%>
			</table>
		</td>
	</tr>
